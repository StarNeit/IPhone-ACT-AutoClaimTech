//
//  CarTopViewController.m
//  ACT
//
//  Created by Sathish Kumar Mariappan on 04/06/12.
...
#import "SaveData.h"
@interface CarTopViewController ()

@end

@implementation CarTopViewController
...
- (void)viewDidLoad
{
    ...
    UIImageView *damagelabel = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"top_text.png"]];
    damagelabel.frame = CGRectMake(40, 80, 179, 28);
   ...
   // [keybtn addTarget:self action:@selector(presskeypad) forControlEvents:UIControlEventTouchUpInside];
    ...
    cartop = [[UIView alloc] initWithFrame:CGRectMake(320, 200, 644, 340)];
    cartop.backgroundColor = [UIColor clearColor];
    [self.view addSubview:cartop];

    UIImageView *car = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"car1.png"]];
    car.frame = CGRectMake(0, 0, 644, 355);
    car.backgroundColor = [UIColor clearColor];
    [cartop addSubview:car];

    carsign4 = [[signview alloc] initWithFrame:CGRectMake(0, 0, 644, 355)];
    carsign4.userInteractionEnabled=YES;
    carsign4.backgroundColor = [UIColor clearColor];
    [cartop addSubview:carsign4];
...
- (UIImage *)captureView:(UIView *)view {
    CGRect screenRect = cartop.bounds;
...
{
    UIImage *shotImg = [ACTAPP_DELEGATE getCarView:ACTScreenShotTopView];
    
    if (shotImg) {
        carsign4.image = shotImg;
    }
    [super viewWillAppear:YES];
}
-(void)viewWillDisappear:(BOOL)animated
{
    UIImage *img = carsign4.image;
    if (img) {
        [ACTAPP_DELEGATE setCarView:carsign4.image forkey:ACTScreenShotTopView];
        [SaveData saveCarImg:[self captureView:cartop] forKey:ACTScreenShotTopView filename:ACTScreenShotTopViewFull];
    } 
}
-(void)clear
{
    carsign4.image = nil;
    [ACTAPP_DELEGATE removeCarView:ACTScreenShotTopView];
    return;
}
...
- (void)rightSwipeHandle:(UISwipeGestureRecognizer*)gestureRecognizer {
    ...
    CarRearViewController *rear =[[CarRearViewController alloc]initWithNibName:@"CarRearViewController" bundle:nil];
    [self.navigationController popViewControllerAnimated:YES];
   // [rear release];
}
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch{
    
    if ([cartop isEqual:touch.view] || [carsign4 isEqual:touch.view]) {
        return NO;
    } else {
        return YES;
    }
